...
/*
  비동기 방식의 코드문제로
  return false 또는 return true 가 실행되지 않는 문제를 해결하기 위해여
  promiss(약속, 보장) 방식으로 코드를 변경한다
...
const st_num_fetch = (st_num) => {
  ...
  // fetch 가 완료된기 전에 실행되어 st_num_fetch 가 종료됨
  // 이 함수가 true나 false 를 return 할 것으로 기대하고
  // 작성을 했으나 비동기 방식의 이유로
  // 중간의 return 명령이 실행될 기회를 잃는다
  return ret;
}; // end st_num_fetch()

// save_cb 함수를 Promiss 방식으로 변경
const save_cb = async () => {
