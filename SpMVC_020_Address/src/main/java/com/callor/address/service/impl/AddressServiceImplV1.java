...
public class AddressServiceImplV1 implements AddressService{
	...
	// 프로젝트가 시작될때 table을 자동으로 생성하기 위한 자동실행 method
	@Autowired
	...
	/*
	 *  이 method 에서 dao 의 create_addr_table() method 를 호출하여
	 *  table 을 생성해도 된다
	 *  
	...
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		long startPageNo = ((searchPage.getCurrentPageNo() - 1 ) / searchPage.getPageNoCoutn()) * searchPage.getPageNoCoutn() +1;
		
		long endPageNo = startPageNo + searchPage.getPageNoCoutn() -1;
		
		searchPage.setStartPageNo(startPageNo);
		searchPage.setEndPageNo(endPageNo);
		searchPage.setLimit(searchPage.getPageNoCoutn());
		searchPage.setOffset(searchPage.getCurrentPageNo() * searchPage.getPageNoCoutn());
