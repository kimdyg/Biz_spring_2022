...
public class UserControllerV2 {
	 ...
	 * 상당히 불편한 상황이 될 것이다
	 * 웹 애플리케이션은 로그인한 사용자 정보를
	 * 어딘가에 보관을 하고 있다가
	 * 
	 * 다른페이지로 이동하였을떄 그 정보를 참조할 수 있어야 한다
	 ...
	 * USER 라는 변수에 login 정보를 저장해 놓는다
	 * 이 후에 서버를 재 시작, 종료 하기 전까지는 USER 라는 변수에는 
	 * loginUser 정보가 계속 담겨 있게 된다.
	 * 
	 * 
	 * 임의로 이 정보를 삭제하려면
	 * session.removeAttribute("USER"); 메서드를 사용하여 USER 변수를
	 * 삭제해 주어야 한다
	 */
	...
	public String join() {
		 ...
		 * /user/join 이다
		 * 
		 * return null을 실행하면 return user/ join 을 실행한 것과 같다
		 * rendering 하라는 의미
		 */
	...
	public String join(UserVO userVO) {
		 ...
		 * => web browse 주소창에 localhost:8080/url
		 */
	 ...
	 * 
	 * id를 email 주소로 사용할때 PathVarriable 로 받을 경우
