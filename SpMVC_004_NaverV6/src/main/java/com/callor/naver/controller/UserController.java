...
public class UserController {
	...
	// UserService interface를 상속받은 어떤 클래스를 주입받겠다 라는 선언
	@Autowired
	 ...
	 * Controller의 각 method는 Spring 으로 부터 HttpSession 객체를 
	 * 주입 받을 수 있다
	 * 
	 * HttpSession 객체에는 client 와 server 간에 상태(status)를 유지하는
	 * 정보를 담을 수 있다
	 *
	 * 여기에 담긴 데이터는 서버가 Runs 하는 동안, 일정 조건을 만족하는 동안
	 * Session 제거하지 않는 한 계속 메모리에 값이 남아있다
	 * 
	 * 큰 데이터를 HttpSession 에 담는것은 썩 좋지 않다
	 * 최소한으로 status 를 유지할 수 있는 데이터만 담는 것이 좋다
	 * 필요가 없어지면 반드시 remove 해 주는것이 좋다
	 * 
